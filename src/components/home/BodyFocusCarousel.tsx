...
const ExerciseCard = memo(function ExerciseCard({ ex, i }: { ex: any; i: number }) {
  return (
    <motion.div
    ...
    >
      <div className="h-24 bg-gradient-card flex items-center justify-center relative overflow-hidden">
        {ex.media_url ? (
          <img src={ex.media_url} alt={ex.name} className="w-full h-full object-cover" loading="lazy" />
        ) : (
    ...
    >
      ...
      <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2">
        {filtered.length === 0 ? (
          ...
          filtered.map((ex, i) => (
            <ExerciseCard key={ex.id} ex={ex} i={i} />
          ))
        )}
      </div>
    </motion.div>
